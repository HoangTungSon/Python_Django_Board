...
urlpatterns = [
    ...
    path('boards/<int:pk>/new', views.new_topics, name='new_topic'),
    ...
    re_path(r'^reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,30}-[0-9A-Za-z]{1,})/$',
    ...
    path('boards/<int:pk>/topics/<int:topic_pk>/reply', views.topic_post_reply, name='topic_posts_reply'),

    path('boards/<int:pk>/topics/<int:topic_pk>/posts/<int:post_pk>/edit/', views.PostUpdateView.as_view(), name='edit_post'),

    path('setting/account/', account_views.UserUpdateView.as_view(), name='my_account'), 
]
